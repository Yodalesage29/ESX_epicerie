...
function setUniform(job, playerPed)
  TriggerEvent('skinchanger:getSkin', function(skin)
    ...
    if skin.sex == 0 then
      if Config.Epicerie[job].male ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Config.Epicerie[job].male)
      else
      ...
      if Config.Epicerie[job].female ~= nil then
        TriggerEvent('skinchanger:loadClothes', skin, Config.Epicerie[job].female)
      else
        ESX.ShowNotification(_U('no_outfit'))
      end
      --if job ~= 'citizen_wear' and job ~= 'barman_outfit' then
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'epicerie', vehicleProps)
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              'default', GetCurrentResourceName(), 'menu_crafting',
              {
                  ...
                  align = 'top-left',
                  elements = {
                      {label = _U('redbull'),          value = 'redbull'},
                      {label = _U('tacos'),            value = 'tacos'},
                      {label = _U('kebab'),            value = 'kebab'},
                      {label = _U('pizza'),            value = 'pizza'},
                      {label = _U('magret de canard'), value = 'magret de canard'},
                      {label = _U('baguette'),         value = 'baguette'},
                      {label = _U('cheeseburger'),     value = 'cheeseburger'},
                      {label = _U('coca cola'),        value = 'coca cola'},
					  {label = _U('ice tea'),          value = 'ice tea'},
                  }
              },
              function(data2, menu2)
            
                TriggerServerEvent('esx_epiceriejob:craftingCoktails', data2.current.value)
        ...
        ESX.UI.Menu.Open(
						'default', GetCurrentResourceName(), 'menumamas_gps',
						{
							...
							elements = {
								...
								{label = 'Récolte Eau', value = 'menumamas_gps_rraisin'},
								{label = 'Vente Eau', value = 'menumamas_gps_vraisin'},
                {label = 'Récolte Pain', value = 'menumamas_gps_rcitron'},
                {label = 'Vente Pain', value = 'menumamas_gps_vcitron'},
                {label = 'STO', value = 'menumamas_gps_rfruit'},
                {label = 'Boucherie', value = 'menumamas_gps_vfruit'},
                {label = 'Supermarché', value = 'menumamas_gps_aliment'},
                {label = 'Port', value = 'menumamas_gps_alcool'}
							},
						},
						function(data2, menu2)

							if data2.current.value == 'menumamas_gps_entreprise' then
								SetNewWaypoint( -3040.652, 589.179, 7.118 )
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_epicerie', _U('billing'), amount)
        end
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_epiceriejob:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_epiceriejob:getStockItem', itemName, count)
            end
...
ESX.TriggerServerCallback('esx_epiceriejob:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_epiceriejob:putStockItems', itemName, count)
            end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_epiceriejob:getFridgeStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_epiceriejob:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_epiceriejob:putFridgeStockItems', itemName, count)
            end
...
function OpenGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_epiceriejob:getVaultWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_epiceriejob:removeVaultWeapon', function()
          OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_epiceriejob:addVaultWeapon', function()
        OpenPutWeaponMenu()
      end, data.current.value)
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'epicerie_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
        function(data, menu)
            TriggerServerEvent('esx_epiceriejob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_epiceriejob:hasEnteredMarker', function(zone)
	...
	if zone == 'EauFarm' then
            if IsJobTrue() then
                CurrentAction     = 'eau_harvest'
                CurrentActionMsg  = _U('press_collect_eau')
                CurrentActionData = {}
            end
    end

    if zone == 'EauResell' then
            if IsJobTrue() then
                --if eauQTE >= 1 then
                    CurrentAction     = 'eau_resell'
                    CurrentActionMsg  = _U('press_sell_eau')
                    CurrentActionData = {}
                --end
            end
    end
	
	if zone == 'PainFarm' then
            if IsJobTrue() then
                CurrentAction     = 'pain_harvest'
                CurrentActionMsg  = _U('press_collect_pain')
                CurrentActionData = {}
            end
    end
	
	if zone == 'PainResell' then
            if IsJobTrue() then
                --if painQTE >= 1 then
                    CurrentAction     = 'pain_resell'
                    CurrentActionMsg  = _U('press_sell_pain')
                    CurrentActionData = {}
                --end
            end
    end
...
AddEventHandler('esx_epiceriejob:hasExitedMarker', function(zone)

    CurrentAction = nil
    ESX.UI.Menu.CloseAll()

	TriggerServerEvent('esx_epiceriejob:stopHarvestEau')
    TriggerServerEvent('esx_epiceriejob:stopSellEau')
    TriggerServerEvent('esx_epiceriejob:stopHarvestPain')
    TriggerServerEvent('esx_epiceriejob:stopSellPain')
	
end)

RegisterNetEvent('esx_epiceriejob:ReturnInventory')
AddEventHandler('esx_epiceriejob:ReturnInventory', function(raisinNbr, citronNbr, fruitNbr, currentZone)
    raisinQTE       = raisinNbr
    citronQTE       = citronNbr
    fruitQTE        = fruitNbr
    ...
    while true do
        ...
        if IsJobTrue() then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_epiceriejob:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent('esx_epiceriejob:hasExitedMarker', LastZone)
            end
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
		...
		if CurrentAction == 'eau_harvest' then
            TriggerServerEvent('esx_epiceriejob:startHarvestEau')
        end
		
		if CurrentAction == 'eau_resell' then
            TriggerServerEvent('esx_epiceriejob:startSellEau')
        end
		
		if CurrentAction == 'pain_harvest' then
            TriggerServerEvent('esx_epiceriejob:startHarvestPain')
        end
		
		if CurrentAction == 'pain_resell' then
            TriggerServerEvent('esx_mamasjob:startSellPain')
        end
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'epicerie', vehicleProps)
            ...
            then
              TriggerServerEvent('esx_service:disableService', 'epicerie')
            end
        ...
        if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
          ...
          TriggerEvent('esx_society:openBossMenu', 'epicerie', function(data, menu)
